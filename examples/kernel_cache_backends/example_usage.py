...
def example_gcp_backend():
    ...
    bucket_name = os.environ.get(
        'CUPY_GCP_BUCKET', 'cupy-kernel-cache-example')
    ...
    try:
        ...
        # The backend automatically falls back to local cache
        # if GCP is unavailable
...
def example_custom_backend():
    ...
    print(f"Loaded data matches saved data: {loaded == test_data}")
...
def example_with_real_kernel():
    ...
    print("Note: Using _set_kernel_cache_backend to change the "
          "global backend.")

    try:
        # Compile with the custom backend (now uses global backend)
        _module = compiler._compile_module_with_cache(
            kernel_source,
            options=()
        )
        ...
        _module2 = compiler._compile_module_with_cache(
            kernel_source,
            options=()
        )
