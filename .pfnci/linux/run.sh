  ...
  for stage in ${STAGES}; do
    ...
    test | shell | benchmark )
      ...
      if [[ ${stage} = test || ${stage} = benchmark ]]; then
        ...
        echo "  - To check env vars automatically set in CI environment: env"
        ...
        echo "  - To build CuPy for AMD ROCm: export CUPY_INSTALL_USE_HIP=1"
        echo "  - To activate ccache for nvcc: export NVCC='ccache nvcc'"
        echo "    (note: ccache is activated by default for gcc/g++)"
