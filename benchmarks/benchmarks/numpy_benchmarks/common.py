...
@memoize
def get_values(xp):
    rnd = xp.random.RandomState(1)
    values = xp.tile(rnd.uniform(0, 100, size=nx*ny//10), 10)
    return values


@memoize
def get_squares(xp):
    values = get_values(xp)
    squares = {t: xp.array(values,
                              dtype=getattr(xp, t)).reshape((nx, ny))
               for t in TYPES1}
...
def get_squares_(xp):
    # smaller squares
    squares_ = {t: s[:nxs, :nys] for t, s in get_squares(xp).items()}
    return squares_
...
def get_vectors(xp):
    # vectors
    vectors = {t: s[0] for t, s in get_squares(xp).items()}
    return vectors
...
def get_indexes(xp):
    ...
    indexes = xp.array(indexes)
    return indexes


@memoize
def get_indexes_rand(xp):
    rnd = random.Random(1)

    indexes_rand = get_indexes().tolist()       # copy
    rnd.shuffle(indexes_rand)         # in-place shuffle
    indexes_rand = xp.array(indexes_rand)
    return indexes_rand
