...
class Eindot(Benchmark):
    ...
    def time_dot_d_dot_b_c(self, np):
        np.dot(self.d, np.dot(self.b, self.c))

    def time_dot_trans_a_at(self, np):
        np.dot(self.a, self.at)

    def time_dot_trans_a_atc(self, np):
        np.dot(self.a, self.atc)

    def time_dot_trans_at_a(self, np):
        np.dot(self.at, self.a)

    def time_dot_trans_atc_a(self, np):
        np.dot(self.atc, self.a)

    def time_einsum_i_ij_j(self, np):
        np.einsum('i,ij,j', self.d, self.b, self.c)

    def time_einsum_ij_jk_a_b(self, np):
        np.einsum('ij,jk', self.a, self.b)

    def time_einsum_ijk_jil_kl(self, np):
        np.einsum('ijk,jil->kl', self.a3, self.b3)

    def time_inner_trans_a_a(self, np):
        np.inner(self.a, self.a)

    def time_inner_trans_a_ac(self, np):
        np.inner(self.a, self.ac)

    def time_matmul_a_b(self, np):
        np.matmul(self.a, self.b)

    def time_matmul_d_matmul_b_c(self, np):
        np.matmul(self.d, np.matmul(self.b, self.c))

    def time_matmul_trans_a_at(self, np):
        np.matmul(self.a, self.at)

    def time_matmul_trans_a_atc(self, np):
        np.matmul(self.a, self.atc)

    def time_matmul_trans_at_a(self, np):
        np.matmul(self.at, self.a)

    def time_matmul_trans_atc_a(self, np):
        np.matmul(self.atc, self.a)

    def time_tensordot_a_b_axes_1_0_0_1(self, np):
